<?php
class class_Load_employee extends class_db {
  function getAllData() {
    $n = array('id' => 'all');
    $sql = "select emp.* ,tm.title_name from employee emp , title_master tm ";
    $sql .= " where (emp.title_id = tm.id)";
  ...
  function getEmployeeByID($emp_id) {
    ...
    $sql = "select emp.* ,tm.title_name from employee emp , title_master tm where (emp_id = :emp_id)";
    $sql .= " and (emp.title_id = tm.id)";
    $result = $this->getAll($sql, $n);
    return $result;
  }
