...
///////////////////////////////////////////
$select_datecheck = "SELECT start_holiday,end_holiday FROM tbl_total_limit_holiday WHERE emp_id ='".$_SESSION["Emp_id"]."' AND status_limit = 1 "; 
...
where detail.status_leave=3 and detail.type_id=1  AND detail.holiday_startdate BETWEEN '$startdate_check' AND '$enddate_check'" ;
...
<body>
    ...
    <td >
     <button type="button" class="btn btn-danger btn" data-toggle="modal" 
        data-target="#<?=$contact1?>" id="<?=$contact1?>" onclick="viewid(<?=$numid?>,
        <?=$numid?>,<?=$numid?>,<?=$featchselect["Emp_id"]?>,<?=$featchselect["hoilday_detail_id"]?>,<?=$featchselect["type_id"]?>)" >ยกเลิก</button>
...
<script type="text/javascript">
    function viewid(num,num1,num2,x,y,z){
