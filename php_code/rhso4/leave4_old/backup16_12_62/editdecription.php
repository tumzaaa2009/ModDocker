...
//////////////////////////////////////////////////////////////////////ดึงชื่อผู้ลาพักร้น
$selectuse="SELECT emp.Emp_id as emp_id,hddetail.hoilday_detail_id AS hd_id,emp.Emp_name as empname ,emp.Emp_lastname 
...
where emp.Emp_id='". $_GET["emp"]."' AND hddetail.hoilday_detail_id='".$_GET["id"]."'  ";
...
if ($_POST["hdncount"]=="") {
  ...
  $countday=$_POST["Total_Day"];
  $counthr=$_POST["Total_Hour"];
  $totalsum=$countday+$counthr;

}else{ // hdn ไม่เท่ากับค่าว่าง
  if ($_POST["hdncount"] >= $sum_old) { //ถ้าค่าใหม่ มากกว่า ค่าเก่า เอาค่าใหม่ลบส่วนต่างค่าเก่า แล้วเอาไปหักออกจากวันลา

        $countday=$_POST["Total_Day"];
           $counthr=$_POST["Total_Hour"];
       $diff = $sum_old-$_POST["hdncount"] ;
       ///เอาค่านี้ไปลบวันลา
  }else { //ถ้าค่าใหม่ น้อยกว่า ค่าเก่า เอาค่าใหม่บวกส่วนต่างค่าเก่า แล้วเอาไปบวกเข้าจากวันลา
  
       $countday=$_POST["Total_Day"];
             $counthr=$_POST["Total_Hour"];
        $diff = $sum_old-$_POST["hdncount"] ;
      ///เอาแค่นี้ไปบวกวันลาเพิ่ม               
      }
      //เช็คค่า เสร็จงานลบทิ้ง
       if ($diff>0) {
        echo 'ลดวันลา'.$diff;
      }else{
        echo 'เพิ่มวันหยุด'.$diff;
      }
...
////////////////////////update วันเข้าออก
$text = "set count_oldyear_hoilday ='".$_POST['totalholiday']."',count_year_holiday='".$_POST['value_total']."',sum_thisyear_holiday ='".$_POST["totalday"]."' " ;  
// echo "<br>",$text;
...
<form method="post" >
 ...
 <tbody>
      <tr >
        <td colspan="8" align="right" class="input_show"><strong>เขียน </strong><strong style="BORDER-BOTTOM: #0C0C0C 1px  dotted;text-align: center;"  > ที่สำนักงานเขตสุขภาพที่ 4</strong> </td>
      </tr>
        <tr >
        <td colspan="8" align="right" class="input_show"><strong>วันที่ </strong><strong style="BORDER-BOTTOM: #0C0C0C 1px  dotted; text-align: center;"  ><?=date("d/m/Y ");?></strong> </td>
      </tr>
      <tr>
        <td colspan="8" align="left"><p class="text-left">เรื่อง ขอลาพักผ่อน</p></td>
      </tr>
      <tr>
        <td colspan="8" align="left"><p class="text-left">เรียน ผู้อำนวยการสำนักงานเขตสุขภาพที่ 4</p></td>
      </tr>
      <tr>
       ...
       <!-- day   -->
       <input type="hidden" name="featchttday" value="<?=$featchuserfrom["hdtotalday"]?>"  id="featchttday">
         <!-- hr  -->
          <input type="hidden" name="featchtthr" value="<?=$featchuserfrom["hdtotalhr"]?>" id="featchtthr">
         <!-- พักผ่อน   -->
          <input type="hidden" id="featchcklimit" name="featchcklimit" value="<?=$featchchecklimitday["count_oldyear_hoilday"]?>">
         
<!-- เก็บค่าเพิม่วัน ถ้า ส่วนต่างมากกว่า 10 ให้ส่วนต่างมาลบ 10 แล้วมาบวกเข้า ปีเก่า -->
<!-- รวมทั้งหมด  -->
 <input type="hidden" name="counttotalday" id="counttotalday" value="<?=$featchchecklimitday["sum_thisyear_holiday"]?>" >

<!-- สว่นต่างที่เป็นค่าvalue -->
<input type="hidden" name="value_total" value="" id="value_total" placeholder="">
...
<!--จบ  -->
        ...
        <td colspan="3"> <input class="form-control" type="text" style="text-align: center;" readonly name="position"
         value="<?=$featchuserfrom["NAME"]?>" placeholder="ตำแหน่ง"></td>
      </tr>
      <tr>
        <td colspan="1">สังกัด</td>
        <td colspan="7" align="left"><input class="form-control" type="text"  style="text-align: center;" readonly value="<?=$featchuserfrom["affname"]?>" name="section" placeholder="สังกัด"></td>
      </tr>
      <tr>
<?php 
...
?>
        ...
        <td colspan="1" ><input class="form-control" type="text"readonly="true" id="totalholiday"  name="totalholiday" 
          value="<?=$featchchecklimitday["count_oldyear_hoilday"]?>" placeholder="วันลาสะสมทั้งหมด"></td>
        <td colspan="1" > วันทำการ</td>
        <td colspan="3" align="center"> <label class="form-check-label" for="materialInline1" > มีสิทธิลาพักร้อนประจำปีนี้อีก 10 วันทำการ รวมเป็น</label></td>
        <td colspan="1"  ><input class="form-control" type="text" id="totalday" name="totalday" readonly="true" value="<?=$featchchecklimitday["sum_thisyear_holiday"]?>" >
...
<?php }?>
      ...
      <tr>
        ...
        <td colspan="1"> <input id="Start_date" name="Start_date" type="date" class="form-control"  required="required" onchange="caltime()"  value="<?=$featchuserfrom["hdstdate"]?>" ></td>
        <td colspan="1" width="10%"> ตั้งแต่เวลา </td>
        <td colspan="1"> 
          <input class="input-group clockpicker" data-placement="top" data-align="left" data-donetext="Done"  id="Start_time" name="Start_time" type="text" value="<?=$featchuserfrom["hdsttime"]?>"    required="required" style="color:#FF0000;" placeholder="Ex.08:30 " onchange="caltime()" > </td>
        <td colspan="1" >ถึงวันที่ </td>
        <td colspan="1"><input id="End_date" name="End_date" onchange="caltime()"type="date" class="form-control"  required="required" value="<?=$featchuserfrom["hdendate"]?>" ></td>
        <td colspan="1"> ถึงเวลา </td>
        <td colspan="1" width="10%" > <input class="input-group clockpicker" data-placement="left" data-align="top"  data-donetext="Done"
            id="End_time" name="End_time" type="text" onchange="caltime()"  required="required" style="color:#FF0000;" placeholder="Ex.16:30 "  value="<?=$featchuserfrom["hdentime"]?>" > </td>
      </tr>
      <tr>
        <td colspan="1">รวมทั้งสิ้น</td>
        <td colspan="1"><input class="form-control" type="text" name="Total_Day" id="Total_Day" required="true" readonly="true" value="<?=$featchuserfrom["hdtotalday"]?>"  ><font >วัน</font></td>
        <td colspan="1"><input class="form-control" type="text" name="Total_Hour" id="Total_Hour" required="true" readonly="true" value="<?=$featchuserfrom["hdtotalhr"]?>" >ชั่วโมง</td>
        <td colspan="1"  width="15%">ในระหว่างลาติดต่อขัาพเจ้าได้ที่</td>
        <td colspan="4"><input class="form-control" type="text" name="contract" placeholder="ติดต่อได้ที่" required="true" value="<?=$featchuserfrom["hdcon"]?>" ></td>
      </tr>
     <tr>
        <td colspan="1">ในระหว่างลานี้ ได้มอบหมายให้ </td>
        <td colspan="7"><input class="form-control" type="text" name="objcontract" placeholder="ได้มอบหมายให้"  required="true" value="<?=$featchuserfrom["hdconobj"]?>" ></td>
     </tr>
  	</tbody>
  </table>
  </div>   
 ...
 if (group_id ==1) {
        if(totalholiday<=10){ //วันหยุดเพิ่ม
          ...
          diff_count//ค่าผลต่าง
                    if (featchcklimit<=10) {
                    document.getElementById("totalholiday").value=  parseFloat(diff_count)+parseFloat(featchcklimit);
                    document.getElementById("totalday").value = parseFloat(test)+parseFloat(diff_count);
                    document.getElementById("value_total").value = 
                    parseFloat( document.getElementById("totalday").value)-parseFloat( document.getElementById("totalholiday").value);
...
document.getElementById("totalholiday").value=discount;
                                ...
                                document.getElementById("totalday").value=parseFloat(  document.getElementById("value_total").value)+discount;
